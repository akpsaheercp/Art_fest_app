    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-amazio-light-bg dark:bg-amazio-bg transition-colors">
        <div className="text-center">
          ...
          <p className="text-zinc-500 font-bold uppercase tracking-widest text-[10px]">Loading Festival Registry...</p>
          <button onClick={logout} className="mt-8 text-xs font-black text-rose-500 uppercase tracking-widest px-4 py-2 border border-rose-500/20 rounded-xl">Logout Session</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen flex font-sans overflow-hidden text-amazio-primary dark:text-zinc-100">
      ...
      {isMobile && !isMobileSticky && (
        <div 
          className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[998] transition-opacity duration-500 ease-in-out ${isSidebarExpanded ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
